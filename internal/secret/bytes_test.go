...
// TestMarshalJSONMethodLiteral asserts the method returns the exact literal
// "\"<redacted>\"" (json.Marshal applied to the value re-escapes "<"/">" per
// encoding/json's default HTML escaping — that is the encoder's behavior, not the
// method's).
...
// carrying a *Bytes redacts to the placeholder and never surfaces the secret.
