...
// non-fatal (the design treats memory locking as best-effort): we return false
// and the caller proceeds. Returns true iff the lock succeeded so memunlock is
// only called when it must be.
