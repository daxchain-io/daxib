...
// baseReq mirrors the keystore-passphrase Request shape (the full env channel
// set) so the precedence ladder can be exercised end to end.
func baseReq() Request {
	return Request{
		EnvFileVar:   "DAXIB_PASSPHRASE_FILE",
		EnvVar:       "DAXIB_PASSPHRASE",
		PromptLabel:  "Keystore passphrase: ",
		RequiredCode: "keystore.passphrase_required",
	}
}

// mnemonicReq mirrors the mnemonic Request shape: NO env channel (stdin/file
// only) and a usage-class RequiredCode.
func mnemonicReq() Request {
	return Request{
...
func TestAcquirePrecedence(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			stdin:      "from-stdin\n",
			env:        map[string]string{"DAXIB_PASSPHRASE": "from-env", "DAXIB_PASSPHRASE_FILE": "/f"},
		...
		},
		{
			...
			env:        map[string]string{"DAXIB_PASSPHRASE": "from-env", "DAXIB_PASSPHRASE_FILE": "/other"},
		...
		},
		{
			...
			env:        map[string]string{"DAXIB_PASSPHRASE": "from-env", "DAXIB_PASSPHRASE_FILE": "/ef"},
		...
		},
		{
			...
			env:        map[string]string{"DAXIB_PASSPHRASE": "from-env"},
			wantSecret: "from-env",
			wantSource: SourceEnv,
		},
		{
			name:       "empty env value is still a present source",
			env:        map[string]string{"DAXIB_PASSPHRASE": ""},
			wantSecret: "",
			wantSource: SourceEnv,
		},
		{
...
// TestAcquirePassphraseRequiredMessage asserts the keystore-passphrase no-source
// error keeps its auth code AND lists the real env channels (no blank holes).
func TestAcquirePassphraseRequiredMessage(t *testing.T) {
	req := baseReq()
	...
	if de.Exit != domain.ExitAuth {
		t.Errorf("exit = %d, want %d (AUTH)", de.Exit, domain.ExitAuth)
	}
	// The message must name the passphrase and BOTH env vars, with no dangling
	// blank slots (the inherited bug interpolated empty env names).
	if !strings.Contains(de.Msg, "Keystore passphrase is required") {
		t.Errorf("message does not name the passphrase: %q", de.Msg)
	}
	if !strings.Contains(de.Msg, "DAXIB_PASSPHRASE_FILE (a file path)") {
		t.Errorf("message omits the *_FILE env channel: %q", de.Msg)
	}
	if !strings.HasSuffix(de.Msg, "DAXIB_PASSPHRASE") {
		t.Errorf("message does not end with the direct env var: %q", de.Msg)
	}
	if strings.Contains(de.Msg, "  ") || strings.HasSuffix(de.Msg, "or ") {
		t.Errorf("message has a blank/dangling env slot: %q", de.Msg)
	}
}

// TestAcquireMnemonicRequiredIsLabelAware is the regression guard for the
// label-aware fix: a missing mnemonic (no env channel) must NOT say "passphrase",
// must NOT advertise any env-var names, and must classify as a usage error (exit
// 2), not the keystore auth class (exit 4).
func TestAcquireMnemonicRequiredIsLabelAware(t *testing.T) {
	req := mnemonicReq()
	_, _, err := Acquire(req, strings.NewReader(""), envMap(nil), func() bool { return false })
	if err == nil {
		t.Fatal("expected a mnemonic-required error")
	}
	var de *domain.Error
	if !errors.As(err, &de) {
		t.Fatalf("error is not a *domain.Error: %T", err)
	...
	if de.Code != domain.CodeMnemonicRequired {
		t.Errorf("code = %q, want %q", de.Code, domain.CodeMnemonicRequired)
	}
	if de.Exit != domain.ExitUsage {
		t.Errorf("exit = %d, want %d (USAGE), not the keystore auth class", de.Exit, domain.ExitUsage)
	}
	if strings.Contains(strings.ToLower(de.Msg), "passphrase") {
		t.Errorf("mnemonic error wrongly mentions 'passphrase': %q", de.Msg)
	}
	if !strings.Contains(de.Msg, "BIP-39 mnemonic is required") {
		t.Errorf("message does not name the mnemonic: %q", de.Msg)
	}
	// No env clauses at all (the mnemonic has no env channel); guard against the
	// blank-name interpolation and the dangling "or ".
	if strings.Contains(de.Msg, "DAXIB_") {
		t.Errorf("mnemonic error must not advertise an env var: %q", de.Msg)
	}
	if strings.Contains(de.Msg, "  ") || strings.HasSuffix(de.Msg, "or ") || strings.Contains(de.Msg, ", ") {
		t.Errorf("mnemonic error has a blank/dangling env slot: %q", de.Msg)
	}
}
...
func TestAcquireEnvFileCheckedToo(t *testing.T) {
	...
	b, src, err := Acquire(
		req,
		strings.NewReader(""),
		envMap(map[string]string{"DAXIB_PASSPHRASE_FILE": "/ef"}),
