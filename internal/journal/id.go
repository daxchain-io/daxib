...
//
// We implement ULID in-package rather than pulling in oklog/ulid: the encoding is
// fully specified and tiny, and keeping it local keeps the journal's dependency
// surface to fsx + domain only (offline-safe, matching daxie).
...
// randReader is the entropy source, swappable in tests so ID generation can be
// made deterministic without touching the rest of the package. Defaults to
// crypto/rand.
var randReader io.Reader = rand.Reader

// nowFn is the wall-clock source for the timestamp half, swappable in tests. It is
// NOT the record timestamp (that comes from the service's injected clock via
// Record.TS) — it only orders IDs — so a provider-local time read here is fine.
var nowFn = time.Now

// newULID returns a fresh ULID string. The timestamp is the current Unix time in
// milliseconds; the 80-bit suffix is crypto-random. A read failure from the
...
func newULIDAt(t time.Time) (string, error) {
	...
	// explicit & 0xFF masks make the byte truncation intentional (the ULID
	// encoding takes the low 8 bits of each shifted group).
	var b [16]byte
	b[0] = byte((ms >> 40) & 0xFF)
	b[1] = byte((ms >> 32) & 0xFF)
	b[2] = byte((ms >> 24) & 0xFF)
