...
// compactSupersededThreshold / compactSizeThreshold are the compaction triggers:
// rewrite the latest-snapshot-per-id when superseded (folded-away) lines exceed
// 5000 OR the file exceeds 8 MiB. Compaction runs under the SAME journal flock as
// the append that triggered it (the caller already holds it).
const (
	compactSupersededThreshold = 5000
	compactSizeThreshold       = 8 << 20 // 8 MiB
)

// readAll reads every record from the network's journal file in seq-append order,
// tolerating torn/corrupt lines: a mid-file non-parsing line is SKIPPED with a
// stderr warning (never fatal); a non-parsing FINAL line (a partial write
...
//
// A missing file (fresh install) is NOT an error — it folds to empty. The caller
// holds the journal flock (exclusive for append, shared for read). repair is true
// ONLY under the exclusive lock (Append/SetState): a torn final line is then
// truncated in place. Under the shared read lock repair is false — the partial
// line is dropped in memory but the file is left for a writer to repair, since
// rewriting under a shared lock could race a concurrent append.
func (s *Store) readAll(net domain.Network, repair bool) (recs []*Record, maxSeq uint64, err error) {
	path := s.filePath(net)
	data, rerr := os.ReadFile(path) // #nosec G304 -- path is the per-network journal file under the state dir
	if rerr != nil {
	...
	for i, line := range lines {
		...
		// A non-newline-terminated FINAL segment is torn by definition (a record is
		// one line = one write(2), terminated by '\n'). A partial write interrupted
		// by a crash leaves the tail un-terminated even if the JSON happens to be
		// complete; drop it and repair the file to the last newline.
		if isFinal && !hasTrailingNL && len(bytes.TrimSpace(line)) > 0 {
			_, _ = fmt.Fprintf(s.warnWriter(), "daxib: journal: dropping torn final line in %s (partial write recovered)\n", path)
			if repair {
		...
		if jerr := json.Unmarshal(trimmed, &r); jerr != nil {
			...
			_, _ = fmt.Fprintf(s.warnWriter(), "daxib: journal: skipping corrupt line %d in %s\n", i+1, path)
			continue
		}
...
// newline, dropping a trailing partial record. It is only called under the
// exclusive flock. It uses fsx.WriteAtomic so the repair is itself crash-safe.
...
// appendLine serializes rec to one line and appends it to the network's journal
// file with the one-write(2) discipline: open the file FRESH by path
// (O_APPEND|O_CREATE — never a long-lived fd, so an append after another
// process's compact-rename lands in the live file, not the unlinked old inode),
// write exactly one line, fsync, close. The caller holds the exclusive flock.
func (s *Store) appendLine(net domain.Network, rec *Record) error {
	...
	path := s.filePath(net)
	f, oerr := os.OpenFile(path, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0o600) // #nosec G304 -- per-network journal file under the state dir
	if oerr != nil {
...
// superseded-line or size threshold is crossed. Terminal records are KEPT (the
// journal IS `tx list` history); only superseded (folded-away) intermediate lines
// are dropped. It runs under the exclusive flock the caller already holds, via
...
func (s *Store) maybeCompact(net domain.Network, preRecs []*Record, latest *Record) error {
	...
	return s.compact(net, folded)
}

// compact writes the folded snapshot (latest line per id, seq-ordered ascending)
// to the journal file via WriteAtomic. Seq values are PRESERVED (not renumbered)
// so foldLatest's "higher seq wins" invariant stays valid; the next append reads
// the preserved max seq and continues. The caller holds the exclusive flock.
func (s *Store) compact(net domain.Network, folded map[string]*Record) error {
	...
	if werr := fsx.WriteAtomic(s.filePath(net), buf.Bytes(), 0o600); werr != nil {
		if fsx.IsReadOnly(werr) {
...
// trailing newline yields a final empty segment which readAll skips; the absence
// of one signals a torn final line.
