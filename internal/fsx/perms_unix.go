...
func permErr(path string, mode os.FileMode) error {
	return domain.WithData(
		domain.Newf(
			...
			"insecure permissions on %s: mode %#o exposes the file to other users; run `chmod 0600 %s` (or set DAXIB_SKIP_PERM_CHECK=1 only on filesystems that cannot represent permissions)",
			path, mode.Perm(), path,
		),
