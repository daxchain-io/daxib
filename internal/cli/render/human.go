...
// typed-error → exit-code MAPPING lives one level up in cli/render.go; this
// subpackage only formats. It imports domain (the wire contract) but no
...
// ErrorEnvelope entry points) and implements neither in M1.
package render
...
// rejects a Renderer interface; a couple of concrete helpers suffice.
type Table struct {
	w    *tabwriter.Writer
	rows int
}
