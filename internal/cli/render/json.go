...
//     (stdout keeps the single-result contract) and ignores the human func.
...
// errEnvelope is the wire shape: {"error":{...}}. domain.Error already marshals
// to {"code","exit","message","retryable","data"}, so we wrap it under the
// "error" key for the on-stderr JSON form.
type errEnvelope struct {
	Error *domain.Error `json:"error"`
}

// ErrorEnvelope writes the error rendering to stderr and returns the exit code
// carried by the error.
//
//   - JSON mode: the structured envelope {"error":{...}} as one line on stderr.
//   - Human mode: a single "daxib: <message> (<code>)" line on stderr.
//
...
func ErrorEnvelope(stderr io.Writer, m Mode, e *domain.Error) domain.ExitCode {
	...
	_, _ = io.WriteString(stderr, "daxib: "+msg+" ("+e.Code+")\n")
	return e.Exit
}
