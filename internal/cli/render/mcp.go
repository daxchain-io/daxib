...
// mcpserver core enumerates; it imports the MCP SDK (third-party, ungoverned by the
// frontend matrix) like the rest of the frontend imports cobra. No network/keystore
// is touched — this is pure formatting.

// mcpToolsList is the --json shape: the tools/list payload {"tools":[…]}. It mirrors
// the SDK's ListToolsResult tools field so `mcp tools --json` and the golden fixture
// marshal identically.
type mcpToolsList struct {
	Tools []*mcp.Tool `json:"tools"`
}

// toolKind classifies a tool for the human table's KIND column from its annotations:
// a ReadOnlyHint tool is "read", otherwise it is a state-changing "sign" tool.
// Data-driven (the annotation the tools package sets), so the renderer needs no
...
// MCPTools writes the `daxib mcp tools` output.
//
...
//   - Human mode: a TOOL / KIND / DESCRIPTION table + the §6.7 footer. The read/sign
//     counts are derived from the tools themselves, so the footer can never disagree
//     with the registered surface.
func MCPTools(w io.Writer, m Mode, tools []*mcp.Tool) error {
	...
	// The footer is essential contract output (the §6.7 wording), so it prints even
	// under --quiet — it is the answer to "what can this server do", not chatter.
	_, _ = fmt.Fprintf(w,
		"\n%d tools (%d read-only, %d signing). Transport: stdio (v1). "+
			"Signing tools enforce policy in core; no policy-mutation, key-export, or backend-mutation tools are exposed.\n",
		len(tools), read, sign)
	return nil
}

// MCPToolSchema writes one tool's full schema for `daxib mcp tools <name>`.
//
