...
//     payload, or one tool's full schema for [<name>]. It builds the server LAZILY and
//     never dials a backend / unlocks the keystore — mcpserver.New touches no
...
// mcpserver.Serve; tools formats what the SDK infers. All tool behavior lives in the
// mcpserver package and the shared service methods.
...
// newMcpServeCmd builds `daxib mcp serve --transport stdio`.
func newMcpServeCmd(ctx context.Context, rs *rootState) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Open the wallet and serve the MCP server until the client disconnects or the\n" +
			...
			"keystore passphrase change (hot-reload is deliberately unsupported).",
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			// Reject http BEFORE opening the service so the unsupported-transport error is
			// fast and side-effect-free (no keystore unlock attempt). The switch is owned
			// by mcpserver.Serve; this is a cheap pre-check that returns the identical
			// domain.Error.
			if transport == "http" {
...
// newMcpToolsCmd builds `daxib mcp tools [<name>]`.
func newMcpToolsCmd(ctx context.Context, rs *rootState) *cobra.Command {
	return &cobra.Command{
		Use:   "tools [name]",
		Short: "List the MCP tools (or one tool's full schema) — never dials a backend",
		Long: "Print the tool surface a connecting MCP client sees. Default: a compact\n" +
			...
			"This command builds the server lazily; it never unlocks the keystore or dials a\n" +
			"backend, so it works in any environment.\n\nExamples:\n" +
			"  daxib mcp tools\n" +
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// introspects over an in-memory pipe. We still open the service so the server
			// is built exactly as `serve` would build it (one assembly path), but no
			// provider is invoked by introspection.
			svc, closeFn, err := openService(ctx, rs)
			if err != nil {
				return err
			}
