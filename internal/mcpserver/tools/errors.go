...
// byte-identical to the CLI --json error (domain.Error.Error()).
//
// These helpers live in package tools (not the mcpserver core) because the handlers
// call them unqualified and because mcpserver imports tools (for Register), so tools
// cannot import mcpserver — putting the helpers here is the cycle-free home.
//
// Three handler paths:
//
...
//	               (tx.wait_timeout: a still-pending tx at the deadline with a
//	               resume command). The SDK fills StructuredContent only on the
//	               nil-error return, so we return a NIL Go error with a hand-built
//	               IsError result + the populated Out (§6.6).
...
// dualSignal reports whether err is one of the §6.6 dual-signal codes: the agent
// must see IsError:true AND the structured *domain.TxResult. For daxib this is the
// tx.wait_timeout outcome — a --wait deadline hit with the tx still pending (exit
// 8, retryable), which is simultaneously an error band AND a fully-populated result
// the agent inspects (status + a resume command). A plain policy denial is NOT
// dual-signal (no TxResult to return).
func dualSignal(err error) bool {
	if err == nil {
		return false
	}
	return domain.AsError(err).Code == domain.CodeTxWaitTimeout
}
...
// error. The TextContent carries the SAME JSON envelope domain.Error.Error() emits,
// byte-identical to the CLI --json error and to the plain-error path above.
