...
//
// Why this matters: the input schemas are INFERRED from the SAME domain request structs
// the CLI binds; the output schemas from the same result structs. So a struct change
// that alters an agent-visible field (a new property, a renamed json tag, a changed
// jsonschema description) shows up HERE as a reviewed diff — CLI/MCP drift is
// structurally impossible AND any drift in the shared structs is caught. This is the
// byte-for-byte contract a client sees on `tools/list` connect, the same payload
// `daxib mcp tools --json` prints (§6.7).
//
// Regenerate after an intentional schema change:
//
//	go test ./internal/mcpserver -run Golden -update
//
// The fixture is REVIEWED in the diff — a surprising change is a bug.

var updateGolden = flag.Bool("update", false, "regenerate the golden tools.json fixture")

const goldenPath = "testdata/tools.json"

// normalizeEOL collapses CRLF to LF so the byte-for-byte golden comparison is stable
// across platforms (a Windows checkout may rewrite line endings). It erases line-ending
// noise only, never content.
...
// (exactly how `daxib mcp tools` introspects lazily, §6.7).
...
// canonJSON marshals v, then re-decodes into an any (recursively, via encoding/json's
// deterministic key-sorted object marshaling) so two structurally-equal schemas
// serialize byte-identically regardless of source key order.
...
func TestToolSchemasGolden(t *testing.T) {
	...
	wantN := normalizeEOL(want)
	gotN := normalizeEOL(gotBytes)
	if !bytes.Equal(wantN, gotN) {
		t.Errorf("MCP tool schema surface drifted from %s.\n"+
			...
			"--- want (%d bytes) ---\n%s\n--- got (%d bytes) ---\n%s",
			goldenPath, len(wantN), string(wantN), len(gotN), string(gotN))
	}
}

// TestGoldenToolCount is a fast guard independent of the byte-diff: the pinned fixture
// (and therefore the live surface) carries EXACTLY the §6.1 tool set, no more, no
// fewer. An accidental extra/dropped tool fails here.
func TestGoldenToolCount(t *testing.T) {
	got := goldenToolsFromServer(t)
	if len(got) != len(wantTools) {
		...
		t.Fatalf("registered tool count = %d, want EXACTLY %d (§6.1); tools: %v", len(got), len(wantTools), names)
	}
}
