#!/bin/sh
# install.sh — universal installer for the `daxib` CLI (the Bitcoin wallet for AI).
#
# Usage:
#
#   curl -fsSL https://github.com/daxchain-io/daxib/releases/latest/download/install.sh | sh
#   curl -fsSL https://github.com/daxchain-io/daxib/releases/latest/download/install.sh | sh -s -- --version v1.0.0
#   curl -fsSL https://github.com/daxchain-io/daxib/releases/latest/download/install.sh | sh -s -- --prefix "$HOME/.local"
#
# Or — recommended — download + inspect first, then run:
#
#   curl -fsSL -o install.sh https://github.com/daxchain-io/daxib/releases/latest/download/install.sh
#   less install.sh
#   sh install.sh
#
# Flags (all optional, env var equivalents in parens — every env is DAXIB_INSTALL_*):
#
#   --version <tag>     Pin to a specific release tag (e.g. v1.0.0). Accepts the tag
#                       with or without a leading 'v'. Default: latest stable.
#                       (env: DAXIB_INSTALL_VERSION)
#   --channel <c>       'stable' (default) or 'beta'. Beta includes -beta.N / -rc.N tags.
#                       (env: DAXIB_INSTALL_CHANNEL)
...
#                       check mandatory. (env: DAXIB_INSTALL_VERIFY_SIGNATURE=1)
#   --dry-run           Print what would happen, change nothing.
#                       (env: DAXIB_INSTALL_DRY_RUN=1)
#   --quiet, -q         Suppress progress output (errors still go to stderr).
#                       (env: DAXIB_INSTALL_QUIET=1)
...
#                       (env: DAXIB_INSTALL_FORCE=1)
#   --uninstall         Remove ONLY the installed binary + the .daxib.install-info marker.
#                       NEVER touches ~/.config/daxib, the keystore, or wallet state.
#   --help, -h          Print this help and exit 0.
#
# Advanced (for local snapshot testing — not part of the public install flow):
#
#   DAXIB_INSTALL_BASE_URL=<url>
#                       Override the release-asset base URL. Default:
#                       https://github.com/daxchain-io/daxib/releases/download/<tag>
...
# and points you at the release .zip archive. Download daxib_<ver>_windows_<arch>.zip from
# the GitHub release and unzip daxib.exe onto your PATH.
#
# Exit codes:
#
#   0   success
...
#
# Repo: https://github.com/daxchain-io/daxib
...
# cosign keyless identity. Daxib pins the EXACT workflow file + tag-ref pattern
# (tighter than a repo-prefix match): the signature must come from this repo's
# release.yml running on a vX.Y.Z[-...] tag. The OIDC issuer is GitHub Actions'
# token endpoint.
...
daxib_version="${DAXIB_INSTALL_VERSION:-}"
daxib_channel="${DAXIB_INSTALL_CHANNEL:-stable}"
daxib_prefix=""
daxib_install_dir="${DAXIB_INSTALL_DIR:-}"
daxib_use_sudo="${DAXIB_INSTALL_USE_SUDO:-}"
daxib_no_verify="${DAXIB_INSTALL_NO_VERIFY:-}"
daxib_verify_sig="${DAXIB_INSTALL_VERIFY_SIGNATURE:-}"
daxib_dry_run="${DAXIB_INSTALL_DRY_RUN:-}"
daxib_quiet="${DAXIB_INSTALL_QUIET:-}"
daxib_force="${DAXIB_INSTALL_FORCE:-}"
daxib_uninstall=""

# DAXIB_INSTALL_BASE_URL is the dev-only escape hatch for pointing the script at
...
daxib_base_url="${DAXIB_INSTALL_BASE_URL:-}"
daxib_base_url="${daxib_base_url%/}"

# ---- io helpers ------------------------------------------------------------

log() {
  if [ -z "$daxib_quiet" ]; then
    printf '%s\n' "$*"
  fi
}

warn() {
  printf 'daxib-install: warning: %s\n' "$*" >&2
}

die() {
  code="$1"
  shift
  printf 'daxib-install: error: %s\n' "$*" >&2
  exit "$code"
}
...
case "$daxib_channel" in
stable | beta) : ;;
*) die 1 "unknown channel '$daxib_channel' (expected 'stable' or 'beta')" ;;
esac
...
if [ -n "$daxib_no_verify" ] && [ -n "$daxib_verify_sig" ]; then
...
# detect_platform  — sets daxib_os (linux|darwin) and daxib_arch (amd64|arm64)
...
detect_platform() {
  ...
  case "$_os" in
  Linux) daxib_os=linux ;;
  Darwin) daxib_os=darwin ;;
  MINGW* | MSYS* | CYGWIN* | Windows*)
    die 2 "Windows is not installable via install.sh — download daxib_<version>_windows_<arch>.zip from https://github.com/${GITHUB_REPO}/releases and unzip daxib.exe onto your PATH"
    ;;
  *) die 2 "unsupported OS: $_os (supported: Linux, Darwin)" ;;
  esac

  case "$_arch" in
  x86_64 | amd64) daxib_arch=amd64 ;;
  aarch64 | arm64) daxib_arch=arm64 ;;
...
# resolve_version  — sets daxib_version to a concrete vX.Y.Z[-suffix] tag. When
...
resolve_version() {
  if [ -n "$daxib_version" ]; then
    # Normalise: allow callers to pass either "v1.0.0" or "1.0.0".
    case "$daxib_version" in
    v*) : ;;
    *) daxib_version="v$daxib_version" ;;
    esac
    return
  fi

  if [ "$daxib_channel" = "stable" ]; then
    ...
    daxib_version="${_final##*/}"
    case "$daxib_version" in
    v*) : ;;
    *) die 3 "could not resolve latest stable tag (got '$daxib_version' from $_final)" ;;
    esac
    return
  fi
  ...
  if ! fetch "$_api" "$_tmp"; then
    rm -f "$_tmp"
    die 3 "failed to query $_api (rate limited? set DAXIB_INSTALL_VERSION=vX.Y.Z to skip the lookup)"
  fi
  # Pluck "tag_name": "vX.Y.Z..." with grep+sed; avoids a jq dependency.
  daxib_version="$(grep -E '"tag_name"' "$_tmp" | head -n 1 | sed -E 's/.*"tag_name"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/')"
  rm -f "$_tmp"
  case "$daxib_version" in
  v*) : ;;
...
# resolve_install_dir  — sets daxib_bindir to the directory we'll mv the binary
# into. Honors --install-dir > --prefix > /usr/local fallback > ~/.local
# fallback. Sets daxib_use_sudo_cmd to "" or "sudo" depending on whether we'll
# need elevation.
resolve_install_dir() {
  daxib_use_sudo_cmd=""

  if [ -n "$daxib_install_dir" ]; then
    daxib_bindir="$daxib_install_dir"
  elif [ -n "$daxib_prefix" ]; then
    daxib_bindir="${daxib_prefix}/bin"
  else
    ...
    if [ -w /usr/local/bin ] || { [ ! -e /usr/local/bin ] && [ -w /usr/local ]; }; then
      daxib_bindir="/usr/local/bin"
    elif [ -n "$daxib_use_sudo" ] && command -v sudo >/dev/null 2>&1; then
      daxib_bindir="/usr/local/bin"
      daxib_use_sudo_cmd="sudo"
    else
      daxib_bindir="${HOME}/.local/bin"
      # No need to mark "didn't exist" — the PATH check at the end of do_install
      # fires whenever daxib_bindir isn't on PATH regardless.
    fi
  fi

  # If the caller pointed us somewhere we can't write, surface that before we
  # download anything.
  if [ -e "$daxib_bindir" ] && [ ! -w "$daxib_bindir" ] && [ -z "$daxib_use_sudo_cmd" ]; then
    die 5 "$daxib_bindir is not writable (re-run with --use-sudo, or pass --prefix=\$HOME/.local)"
  fi
}

# ---- download + verify ------------------------------------------------------

# download_and_verify  — fetches the release archive matching daxib_os /
# daxib_arch / daxib_version into daxib_workdir and (unless --no-verify) checks
...
# BIN_NAME file landed in daxib_workdir.
download_and_verify() {
  # goreleaser archive name template:
  #   daxib_<version_no_v>_<os>_<arch>.tar.gz
  _ver_noprefix="${daxib_version#v}"
  _archive="${BIN_NAME}_${_ver_noprefix}_${daxib_os}_${daxib_arch}.tar.gz"
  _checksums="checksums.txt"

  if [ -n "$daxib_base_url" ]; then
    ...
    _base="$daxib_base_url"
  else
    _base="https://github.com/${GITHUB_REPO}/releases/download/${daxib_version}"
  fi
  _archive_url="${_base}/${_archive}"
  _checksums_url="${_base}/${_checksums}"

  log "Downloading ${_archive_url}"
  if ! fetch "$_archive_url" "${daxib_workdir}/${_archive}"; then
    die 3 "failed to download ${_archive_url} (does the tag '${daxib_version}' have a ${daxib_os}/${daxib_arch} build?)"
  fi

  if [ -z "$daxib_no_verify" ]; then
    log "Verifying SHA256"
    if ! fetch "$_checksums_url" "${daxib_workdir}/${_checksums}"; then
    ...
    _line="$(grep " ${_archive}\$" "${daxib_workdir}/${_checksums}" || true)"
    if [ -z "$_line" ]; then
      die 4 "checksums.txt has no entry for ${_archive}"
    fi
    printf '%s\n' "$_line" >"${daxib_workdir}/${_archive}.sha256"
    (cd "$daxib_workdir" && $sha256_cmd -c "${_archive}.sha256") >/dev/null 2>&1 ||
  ...
  if [ -z "$daxib_no_verify" ]; then
    if command -v cosign >/dev/null 2>&1; then
      ...
      if [ ! -f "${daxib_workdir}/${_checksums}" ]; then
        fetch "$_checksums_url" "${daxib_workdir}/${_checksums}" ||
          die 4 "failed to download checksums.txt from $_checksums_url"
      fi
      _bundle_url="${_base}/${_checksums}.sigstore.json"
      if fetch "$_bundle_url" "${daxib_workdir}/${_checksums}.sigstore.json"; then
        log "Verifying cosign keyless signature on checksums.txt"
        cosign verify-blob \
          --bundle "${daxib_workdir}/${_checksums}.sigstore.json" \
          --certificate-identity-regexp "$COSIGN_IDENTITY_REGEXP" \
          --certificate-oidc-issuer "$COSIGN_OIDC_ISSUER" \
          "${daxib_workdir}/${_checksums}" >/dev/null 2>&1 ||
          die 4 "cosign signature verification failed (identity must match this repo's release workflow)"
      elif [ -n "$daxib_verify_sig" ]; then
  ...
  (cd "$daxib_workdir" && tar -xzf "$_archive")
  if [ ! -f "${daxib_workdir}/${BIN_NAME}" ]; then
...
# check_existing_install  — if <daxib_bindir>/<BIN_NAME> exists AND its marker
...
check_existing_install() {
  [ -n "$daxib_force" ] && return 0
  [ -e "${daxib_bindir}/${BIN_NAME}" ] || return 0

  _existing="$(read_marker_version "$daxib_bindir")"
  if [ -z "$_existing" ]; then
    ...
    warn "${daxib_bindir}/${BIN_NAME} exists but has no install marker — replacing it."
    return 0
  fi

  if [ "$_existing" = "$daxib_version" ]; then
    log "${BIN_NAME} ${daxib_version} is already installed at ${daxib_bindir}/${BIN_NAME}."
    log "Pass --force (or set DAXIB_INSTALL_FORCE=1) to reinstall."
    # Exit cleanly so curl|sh callers don't see a non-zero status. The trap
    # cleans up the workdir on the way out.
    exit 0
  fi

  log "Upgrading ${BIN_NAME}: ${_existing} → ${daxib_version}"
}

# ---- install ----------------------------------------------------------------

# do_install  — moves the verified binary from daxib_workdir to
# ${daxib_bindir}/${BIN_NAME} atomically (cp + chmod 0755 + mv onto a sibling
...
# --use-sudo, and emits a shell-aware PATH advisory if daxib_bindir isn't on
# PATH.
do_install() {
  _dest="${daxib_bindir}/${BIN_NAME}"
  _tmp_dest="${daxib_bindir}/.${BIN_NAME}.new.$$"

  if [ -n "$daxib_dry_run" ]; then
    log "[dry-run] would install ${daxib_workdir}/${BIN_NAME} → ${_dest}"
    log "[dry-run] would write ${daxib_bindir}/${MARKER_NAME}"
    return
  fi

  # Make sure the bin dir exists. mkdir -p; if that needs sudo the caller
  # already opted in via --use-sudo (daxib_use_sudo_cmd is set). Otherwise bail
  # with a clear message.
  if [ ! -d "$daxib_bindir" ]; then
    if ! ${daxib_use_sudo_cmd} mkdir -p "$daxib_bindir" 2>/dev/null; then
      die 5 "cannot create $daxib_bindir — re-run with --use-sudo or --prefix=\$HOME/.local"
    fi
  fi
  ...
  ${daxib_use_sudo_cmd} cp "${daxib_workdir}/${BIN_NAME}" "$_tmp_dest"
  ${daxib_use_sudo_cmd} chmod 0755 "$_tmp_dest"
  ${daxib_use_sudo_cmd} mv "$_tmp_dest" "$_dest"

  # Drop a sibling marker recording how this binary was installed. Keep the
  # schema forward-compatible: simple key=value lines.
  _marker="${daxib_bindir}/${MARKER_NAME}"
  _marker_tmp="${daxib_workdir}/marker.$$"
  {
  ...
  ${daxib_use_sudo_cmd} mv "$_marker_tmp" "$_marker"

  log "Installed ${BIN_NAME} ${daxib_version} → ${_dest}"

  # Run the binary so the user gets the same `daxib version` line they'd see
  # otherwise — confirms the install works (right arch, not corrupted) and shows
  # the commit + build date inline. Don't fail the overall install on a `daxib
  ...
  *)
    log ""
    warn "${daxib_bindir} is not on your PATH."
    warn "Add it for the current shell:"
    warn "  export PATH=\"${daxib_bindir}:\$PATH\""
    _shell_name=""
    [ -n "${SHELL:-}" ] && _shell_name="$(basename "$SHELL")"
    case "$_shell_name" in
    bash)
      warn "Persist it (bash):"
      warn "  echo 'export PATH=\"${daxib_bindir}:\$PATH\"' >> ~/.bashrc"
      ;;
    zsh)
      warn "Persist it (zsh):"
      warn "  echo 'export PATH=\"${daxib_bindir}:\$PATH\"' >> ~/.zshrc"
      ;;
    fish)
      warn "Persist it (fish):"
      warn "  fish_add_path ${daxib_bindir}"
      ;;
    *)
      warn "Persist it (pick the file matching your shell):"
      warn "  echo 'export PATH=\"${daxib_bindir}:\$PATH\"' >> ~/.bashrc"
      warn "  echo 'export PATH=\"${daxib_bindir}:\$PATH\"' >> ~/.zshrc"
      warn "  fish_add_path ${daxib_bindir}    # fish"
      ;;
    esac
    ;;
  esac
...
do_uninstall() {
  # Locate a previously-installed daxib. Prefer the marker file; otherwise fall
  # back to `command -v daxib`.
  ...
  _sudo=""
  if [ ! -w "$_found" ] && [ -n "$daxib_use_sudo" ] && command -v sudo >/dev/null 2>&1; then
    _sudo="sudo"
  fi

  if [ -n "$daxib_dry_run" ]; then
    log "[dry-run] would remove ${_found}/${BIN_NAME} and ${_found}/${MARKER_NAME}"
    log "[dry-run] would NOT touch ~/.config/daxib (keystore, policy, state)"
    return
  fi
  ...
  # defect. No flag of this script ever deletes key material.
  ${_sudo} rm -f "${_found}/${BIN_NAME}" "${_found}/${MARKER_NAME}"
  log "Uninstalled ${BIN_NAME} from ${_found}"
  log "Note: config, keystore, and state under ~/.config/daxib are NOT removed."
  log "      (To wipe them yourself — IRREVERSIBLE, includes key material: rm -rf ~/.config/daxib)"
}

# ---- main -------------------------------------------------------------------

main() {
  if [ -n "$daxib_uninstall" ]; then
    do_uninstall
    return
  fi
  ...
  log "Installing ${BIN_NAME} ${daxib_version} (${daxib_os}/${daxib_arch}) → ${daxib_bindir}"
  if [ -n "$daxib_use_sudo_cmd" ]; then
    log "Using sudo for writes to ${daxib_bindir}"
  fi

  # Workdir: per-run temp dir cleaned up on exit (success or failure).
  daxib_workdir="$(mktemp -d 2>/dev/null || mktemp -d -t "${BIN_NAME}-install")"
  # shellcheck disable=SC2064  # intentional early-binding of $daxib_workdir
  trap "rm -rf '$daxib_workdir'" EXIT INT HUP TERM

  if [ -n "$daxib_dry_run" ]; then
    log "[dry-run] would download ${BIN_NAME} ${daxib_version} for ${daxib_os}/${daxib_arch}"
    do_install
    return
  fi
  ...
  download_and_verify
  do_install
  # do_install execs the binary itself, so no separate "Run 'daxib version' to
