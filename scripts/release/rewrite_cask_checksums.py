...
URL_RE = re.compile(
    r'^\s*url "https://github\.com/daxchain-io/daxib/releases/download/'
    r'v#\{version\}/daxib_#\{version\}_(?P<suffix>[^"]+)"\s*$'
)
...
def rewrite(cask_path: Path, checksums: dict[str, str], version: str) -> tuple[str, list[str], bool]:
    ...
    for index, line in enumerate(lines):
        ...
        filename = f"daxib_{version}_{suffix}"
    ...
    required = {f"daxib_{version}_{suffix}" for suffix in REQUIRED_SUFFIXES}
